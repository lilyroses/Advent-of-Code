...
def create_calendar_line(days_per_row, day_items, day_width):
    """
    - day_items : a list of strings, one for each day, to be placed in the day squares.
    """
    if not day_items:
    ...
    for item in day_items:
        ...
        num_left_spaces = (cell_spaces // 2)
        num_right_spaces = (cell_spaces - num_left_spaces)
        cell_interior = PIPE + (num_left_spaces * SPACE) + item + (num_right_spaces * SPACE)
...
row = create_calendar_line(5, [1,2,3,4,5], day_width)
row = f"""
{row}
"""
print(row)
